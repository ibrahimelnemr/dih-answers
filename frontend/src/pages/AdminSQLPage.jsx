...
export default function AdminSQLPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">SQL Console</h1>
      ...
      <form onSubmit={handleExecute} className="space-y-4">
        <div>
          <label htmlFor="sql" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            SQL Query
          </label>
          <textarea
            id="sql"
            ...
            className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 font-mono text-sm text-gray-900 dark:text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500"
          />
        </div>
        <button
          type="submit"
      ...
      {result && (
        <div className="space-y-2">
          {result.message && (
            <p className="text-sm text-brand-700 dark:text-brand-400 font-medium">{result.message}</p>
          )}
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                  {result.rows.map((row, i) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-50 dark:bg-gray-800/50"}>
                      {row.map((cell, j) => (
                        <td key={j} className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 font-mono whitespace-nowrap">
